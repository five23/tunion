...
function button(target, transmitCommand) {
    ...
    with (self.context) {
      ...
      if (imageButton) {
        ...
        arc(
          self.center.x,
          self.center.y,
          Math.min(self.center.x, self.center.y) - self.lineWidth / 2,
          0,
          Math.PI * 2,
          true
        );
        ...
        if (self.clicked && self.mode == "node") {
          ...
          arc(
            self.val.x,
            self.val.y,
            Math.min(self.center.x, self.center.y) / 2,
            0,
            Math.PI * 2,
            true
          );
  ...
  this.setHoverImage = function (image) {
    ...
    self.imageHover.onload = function () {
      self.draw();
    };
  ...
  this.setTouchImage = function (image) {
    ...
    self.imageTouch.onload = function () {
      self.draw();
    };
...
function colors(target, transmitCommand) {
  ...
  this.drawColor = function () {
    with (self.context) {
      fillStyle =
        "rgb(" + self.val.r + "," + self.val.g + "," + self.val.b + ")";
      beginPath();
      arc(
        self.width / 8,
...
function envelope(target, transmitCommand) {
  ...
  this.draw = function () {
    ...
    with (self.context) {
      ...
      var drawingX =
        self.val.x * self.actualWid + self.nodeSize + self.lineWidth;
      var drawingY =
        self.val.y * self.actualHgt + self.nodeSize + self.lineWidth;
...
function joints(target, transmitCommand) {
  ...
  this.scaleNode = function () {
    self.values = [
      nx.prune(self.val.x / self.width, 3),
      nx.prune(self.val.y / self.height, 3),
    ];
    return self.values;
  };
  ...
  this.aniBounce = function () {
    if (!self.clicked && self.val.x) {
      ...
      self.deltaMove.x = nx.bounce(
        self.val.x,
        self.bgLeft + self.nodeSize,
        self.width - self.bgLeft - self.nodeSize,
        self.deltaMove.x
      );
      self.deltaMove.y = nx.bounce(
        self.val.y,
        self.bgTop + self.nodeSize,
        self.height - self.bgTop - self.nodeSize,
        self.deltaMove.y
      );
      self.draw();
...
function metroball(target, transmitCommand) {
  ...
  this.createUISpaces = function () {
    ...
    for (var i = 0; i < self.UISpaces.length; i++) {
      ...
      self.UISpaces[i].centerx =
        self.UISpaces[i].xpos + self.UISpaces[i].wid / 2;
      self.UISpaces[i].centery =
        self.UISpaces[i].ypos + self.UISpaces[i].hgt / 2;
    }
  };
  ...
  this.addNewMB = function (ballPos) {
    ...
    self.CurrentBalls[nextIndex] = new self.Ball(
      nextIndex,
      ballPos.x,
      ballPos.y
    );
  ...
  this.Ball = function (SelfIndex, SelfX, SelfY) {
    ...
    this.move = function () {
      ...
      if (
        this.ypos > this.space.ypos2 - this.size - 2 ||
        this.ypos < this.space.ypos + this.size + 2
      ) {
        this.bounce();
      }
...
function mouse(target, transmitCommand) {
  ...
  this.init = function () {
    ...
    self.mousing = window.addEventListener(
      "touchmove",
      self.preTouchMove,
      false
    );
  ...
  this.draw = function () {
    ...
    with (self.context) {
      ...
      fillRect(
        self.inside.quarterwid * 2,
        self.inside.height,
        self.inside.quarterwid,
        scaleddx
      );
      fillRect(
        self.inside.quarterwid * 3,
        self.inside.height,
        self.inside.quarterwid,
        scaleddy
      );
      ...
      fillText(
        "x",
        self.inside.quarterwid * 0 + self.inside.quarterwid / 2,
        self.height - 7
      );
      fillText(
        "y",
        self.inside.quarterwid * 1 + self.inside.quarterwid / 2,
        self.height - 7
      );
      fillText(
        "dx",
        self.inside.quarterwid * 2 + self.inside.quarterwid / 2,
        self.height - 7
      );
      fillText(
        "dy",
        self.inside.quarterwid * 3 + self.inside.quarterwid / 2,
        self.height - 7
      );
  ...
  this.move = function (e) {
    self.val = {
      deltax:
        (e.clientX - document.body.scrollLeft) / window.innerWidth - self.val.x,
      deltay:
        (e.clientY - document.body.scrollTop) / window.innerHeight - self.val.y,
  ...
  this.draw = function () {
    ...
    with (this.context) {
      ...
      for (i = 0; i < self.sliders; i++) {
        beginPath();
        moveTo(
          self.padding + i * self.sliderWidth,
          self.height - self.val[i] * self.height
        );
        lineTo(
          self.padding + i * self.sliderWidth + self.sliderWidth,
          self.height - self.val[i] * self.height
        );
        stroke();
        lineTo(
      ...
      if (self.clickPos.touches.length > 1) {
        for (var i = 0; i < self.clickPos.touches.length; i++) {
          var sliderToMove = Math.floor(
            self.clickPos.touches[i].x / self.sliderWidth
          );
          sliderToMove = nx.clip(sliderToMove, 0, self.sliders - 1);
          self.val[sliderToMove] = nx.clip(
            nx.invert(self.clickPos.touches[i].y / self.height),
            0,
            1
          );
        }
        ...
        sliderToMove = nx.clip(sliderToMove, 0, self.sliders - 1);
        self.val[sliderToMove] = nx.clip(
          nx.invert(self.clickPos.y / self.height),
          0,
          1
        );
        ...
        for (var j = 0; j < self.rows; j++) {
          for (var i = 0; i < self.cols; i++) {
            with (self.context) {
              ...
              lineWidth = 1;
              var circx =
                (i * self.width) / self.cols + self.width / self.cols / 2;
              var circy =
                (j * self.height) / self.rows + self.height / self.rows / 2;
              arc(
                circx,
                circy,
              ...
              if (self.matrixLabels) {
                ...
                //fillText((10-j)*(i+1), circx, circy);
                fillText(
                  self.matrixLabels[count % self.matrixLabels.length],
                  circx,
                  circy
                );
        ...
        if (self.clickPos.touches.length >= 1) {
          for (var i = 0; i < self.clickPos.touches.length; i++) {
            with (self.context) {
              ...
              lineWidth = self.lineWidth;
              arc(
                self.clickPos.touches[i].x,
                self.clickPos.touches[i].y,
                self.nodeSize,
                0,
                Math.PI * 2,
                true
...
function number(target, transmitCommand) {
  ...
  this.draw = function () {
    ...
    with (self.context) {
      ...
      fillStyle = self.colors.black;
      textAlign = "left";
      font = self.height * 0.6 + "px courier";
      textBaseline = "middle";
...
function range(target, transmitCommand) {
  ...
  this.init = function () {
    ...
    this.realSpace = {
      x: self.width - self.lineWidth * 2,
      y: self.height - self.lineWidth * 2,
    };
  ...
  this.click = function () {
    if (self.hslider) {
      if (
        Math.abs(self.clickPos.x - self.val.start * self.width) <
        Math.abs(self.clickPos.x - self.val.stop * self.width)
      ) {
      ...
      if (
        Math.abs(
          Math.abs(self.clickPos.y - self.height) - self.val.start * self.height
        ) <
        Math.abs(
          Math.abs(self.clickPos.y - self.height) - self.val.stop * self.height
        )
      ) {
        self.firsttouch = "start";
      } else {
  ...
  this.move = function () {
    ...
    if (self.clicked) {
      ...
      self.val["size"] = nx.prune(
        nx.clip(Math.abs(self.val.stop - self.val.start), 0, 1),
        3
      );
...
function sandbox(target, transmitCommand) {
  ...
  this.isInsideCircle = function (clickedNode, currObject) {
    if (
      clickedNode.x > currObject.xpos - currObject.wid / 2 &&
      clickedNode.x < currObject.xpos + currObject.wid / 2 &&
      clickedNode.y > currObject.ypos - currObject.hgt / 2 &&
      clickedNode.y < currObject.ypos + currObject.hgt / 2
    ) {
      return true;
    } else {
  ...
  this.createUISpaces = function () {
    ...
    for (i = 0; i < this.UISpaces.length; i++) {
      ...
      this.UISpaces[i].centerx =
        this.UISpaces[i].xpos + this.UISpaces[i].wid / 2;
      this.UISpaces[i].centery =
        this.UISpaces[i].ypos + this.UISpaces[i].hgt / 2;
    }
  };
  ...
  this.move = function () {
    ...
    if (self.mode == "absolute") {
      ...
      if (self.clicked) {
        if (!self.hslider) {
          self.val = nx.clip(
            self.val + (self.deltaMove.y * -1) / self.height,
            0,
            1
          );
        } else {
...
function string(target, transmitCommand) {
      ...
      for (var i = 0; i < self.strings.length; i++) {
        ...
        if (st.vibrating) {
          ...
          if (Math.abs(st.stretch) > st.maxstretch) {
            ...
            st.direction =
              (st.direction / Math.abs(st.direction)) * (st.maxstretch / 1);
          }
  ...
  this.move = function () {
    if (self.clicked) {
      for (var i = 0; i < self.strings.length; i++) {
        ...
        if (
          self.strings[i].held &&
          Math.abs(self.clickPos.y - self.strings[i].y1) >
            self.height / (self.strings.length * 3)
        ) {
          self.pluck(i);
        }
      }
  ...
  this.pluck = function (which) {
    ...
    self.strings[i].direction =
      ((self.clickPos.y - self.strings[i].y1) /
        Math.abs(self.clickPos.y - self.strings[i].y1)) *
      ((self.clickPos.y - self.strings[i].y1) / -1.2);
  };
}
/**
	@class toggle
  ...
  this.draw = function () {
    ...
    if (self.width > 40 && self.height > 40) {
      if (this.val) {
        nx.makeRoundRect(
          this.context,
          this.bgLeft + this.padding,
          this.bgTop + this.padding,
          this.bgWidth - this.padding * 2,
          this.bgHeight / 2.1
        );
        with (this.context) {
          fillStyle = self.colors.accent;
          ...
          fillText(
            "on",
            this.canvas.width / 2,
            this.bgHeight / 4.5 + this.lineWidth + this.padding + 5
          );
        }
      } else {
        nx.makeRoundRect(
          this.context,
          this.bgLeft + this.padding,
          this.bgBottom - this.padding - this.bgHeight / 2.1,
          this.bgWidth - this.padding * 2,
          this.bgHeight / 2.1
        );
        with (this.context) {
          ...
          fillText(
            "off",
            this.canvas.width / 2,
            this.bgBottom - this.padding - this.bgHeight / 4.5 + 5
          );
        }
      }
    } else {
      with (this.context) {
        ...
        if (self.val) {
          fillText(
            "on",
            this.canvas.width / 2,
            this.canvas.height / 2 + self.fontsize / 3.5
          );
        } else {
          fillText(
            "off",
...
function tilt(target, transmitCommand) {
  ...
  this.draw = function () {
    ...
    with (self.context) {
      ...
      fillStyle = self.colors.fill;
      lineWidth = self.lineWidth;
      var grd = self.context.createRadialGradient(
        self.width / 3,
        self.height / 5,
        self.width / 20,
        self.width / 3,
      ...
      fillRect(
        -self.width,
        self.height * (self.val.y / 2) + self.height / 2,
        self.width * 3,
        self.height * 2
      );
      ...
      fillText(
        self.text,
        self.width / 2,
        self.height * (self.val.y / 2) + self.height / 2 + self.height / 15
      );
      globalAlpha = 1;
...
function typewriter(target, transmitCommand) {
  ...
  this.rows = [
    ...
    [
      ...
      { symbol: "opt", value: 10, width: 1, on: false },
      { symbol: "<", value: 37, width: 0.81, on: false },
      { symbol: "^", value: 38, width: 0.81, on: false },
      { symbol: "v", value: 39, width: 0.81, on: false },
      { symbol: ">", value: 40, width: 0.81, on: false },
    ],
  ];
      ...
      for (var i = 0; i < self.rows.length; i++) {
        ...
        for (var j = 0; j < self.rows[i].length; j++) {
          ...
          nx.makeRoundRect(
            ...
            currkeyL,
            i * self.keyhgt,
            self.keywid * self.rows[i][j].width,
            self.keyhgt,
            8
          );
...
function vinyl(target, transmitCommand) {
  ...
  this.click = function (e) {
    ...
    self.grabPos = nx.toPolar(
      self.clickPos.x - self.center.x,
      self.clickPos.y - self.center.y
    ).y;
  };
  ...
  this.move = function () {
    ...
    self.speed =
      (self.rotation -
        self.lastRotation +
        (self.lastRotation - self.lastRotation2)) /
      2;
  ...
  this.draw = function () {
    with (self.context) {
      ...
      for (var i = 0; i < self.spokes; i++) {
        var dot = nx.toCartesian(
          self.circleSize - 5,
          (i / self.spokes) * Math.PI * 2 -
            self.rotation +
            (Math.PI * 2) / (self.spokes * 2)
        );
        beginPath();
        arc(
          dot.x + self.center.x,
  ...
  this.move = function () {
    ...
    if (
      ...
      for (var i = 0; i < self.spokes; i++) {
        ...
        if (
          self.rotation - (i / self.spokes) * Math.PI * 2 > 0 &&
          self.lastRotation - (i / self.spokes) * Math.PI * 2 < 0
        ) {
          self.val = i;
          self.nxTransmit(self.val);
        }
...
var nx = function () {
  ...
  this.printOSC = function () {
    $("body").append(
      '<div style="background-color:#eee;padding:10px;margin-top:20px;font-family:courier;font-size:12pt">OSC Output <input type="button" value="View Code for this UI" onclick=\'window.open("view-source:" + window.location.href)\' style="font-size:12pt;float:right"></input></div>'
    );
    $("body").append(
      '<div id="debug" style="padding:10px;clear:both;height:630px;font-size:15pt;background-color:#ddd;color:#777;overflow:hidden;font-family:courier"></div>'
    );
    self.logOSC = true;
  };
  ...
  this.nxTransmit = function (data) {
    ...
    if (this.transmissionProtocol == "none") {
      ...
      this.transmissionProtocol == "local" ||
  ...
  this.globalLocalTransmit = function (
    localObjectFrom,
    localObject,
    localParameter,
    data
    ...
    if (typeof data == "object" && data !== null) {
      for (var key in data) {
        if (typeof data[key] == "object" && data[key] !== null) {
          ...
          var osc_message =
            "nexus://default?" + this.oscName + "/" + key + "=" + data[key];
          window.location.href = osc_message;
        }
      }
    } else if (typeof data == "number" || typeof data == "string") {
  ...
  this.getHandlers = function (self) {
    if (manager.is_touch_device) {
      ...
      self.canvas.ontouchmove = self.nxThrottle(
        self.preTouchMove,
        self.nxThrottlePeriod
      );
  ...
  this.isInside = function (clickedNode, currObject) {
    if (
      clickedNode.x > currObject.xpos &&
      clickedNode.x < currObject.xpos + currObject.wid &&
      clickedNode.y > currObject.ypos &&
      clickedNode.y < currObject.ypos + currObject.hgt
    ) {
      return true;
    } else {
      return false;
    ...
    if (nxId) {
      eval(
        allcanvi[i].id +
          ...
          "('" +
          allcanvi[i].id +
          "', '../../servers/nexusPHP/nexusOSCRelay.php', " +
          idNum +
          ");"
      );
...
function getTemplate(self, target, transmitCommand) {
  ...
  self.offset = new nx.canvasOffset(
    nx.findPosition(self.canvas).left,
    nx.findPosition(self.canvas).top
  );
  self.center = {
    x: self.width / 2,
    y: self.height / 2,
  ...
  self.preClick = function (e) {
    ...
    if (nx.editmode) {
      if (
        self.clickPos.x > self.width - 20 &&
        self.clickPos.y > self.height - 20
      ) {
  ...
  self.preMove = function (e) {
    ...
    if (nx.editmode) {
      if (self.isBeingResized) {
        ...
        self.canvas.width =
          ~~(self.clickPos.x / (canvasgridx / 2)) * (canvasgridx / 2);
        self.canvas.height =
          ~~(self.clickPos.y / (canvasgridy / 2)) * (canvasgridy / 2);
        ...
        var matrixy =
          ~~((e.pageY - self.height / 2) / canvasgridy) * canvasgridy;
        var matrixx =
          ~~((e.pageX - self.width / 2) / canvasgridx) * canvasgridx;
    ...
    if (self.clicked) {
      ...
      if (nx.editmode) {
        if (self.isBeingDragged) {
          var matrixy =
            ~~((e.targetTouches[0].pageY - self.height / 2) / canvasgridy) *
            canvasgridy;
          var matrixx =
            ~~((e.targetTouches[0].pageX - self.width / 2) / canvasgridx) *
            canvasgridx;
          self.canvas.style.top = matrixy + "px";
          self.canvas.style.left = matrixx + "px";
          self.offset = new nx.canvasOffset(
          ...
          self.canvas.width =
            ~~(e.targetTouches[0].pageX / (canvasgridx / 2)) *
            (canvasgridx / 2);
          self.canvas.height =
            ~~(e.targetTouches[0].pageY / (canvasgridy / 2)) *
            (canvasgridy / 2);
